...
from tetris_gymnasium.functional.queue import create_bag_queue, bag_queue_get_next_element
import jax
...
class TetrisVisualizer:
    ...
    def render(self) -> np.ndarray:
        ...
        for i in range(self.config.height):
            for j in range(self.config.width):
                if board[i, j] > 0:
                    ...
                    rgb_color = tuple(map(int, TETROMINOES.colors[board[i, j] - 2]))  # -2 because piece IDs start at 2
        ...
        for i in range(self.config.height + 1):
            cv2.line(image, (0, i * self.cell_size), (self.window_width, i * self.cell_size), grid_color, 1)
        for j in range(self.config.width + 1):
            cv2.line(image, (j * self.cell_size, 0), (j * self.cell_size, self.window_height), grid_color, 1)
