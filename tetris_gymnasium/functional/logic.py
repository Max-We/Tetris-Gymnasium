...
def create_board(config: EnvConfig, tetrominoes: Tetrominoes) -> chex.Array:
    ...
    return jnp.pad(board, ((0, config.padding), (config.padding, config.padding)), mode='constant', constant_values=tetrominoes.base_pixels[1])
...
def clear_filled_rows(config: EnvConfig, tetrominoes: Tetrominoes, board: chex.Array) -> Tuple[chex.Array, int]:
...
def commit_active_tetromino(config: EnvConfig, tetrominoes: Tetrominoes, state: State, key: chex.PRNGKey) -> Tuple[chex.PRNGKey, State, float, bool]:
    tetromino_matrix = get_tetromino_matrix(tetrominoes, state.active_tetromino, state.rotation)
    board = project_tetromino(state.board, tetromino_matrix, state.x, state.y, tetrominoes.ids[state.active_tetromino])
    board, lines_cleared = clear_filled_rows(config, tetrominoes, board)
    ...
    game_over = collision(board, get_tetromino_matrix(tetrominoes, new_active_tetromino, new_rotation), new_x, new_y)
